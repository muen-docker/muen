...
is
   ...
   type VMCS_Seg_Fields_Type is record
      Selector_Field      : SK.Word16;
      Base_Field          : SK.Word16;
      Limit_Field         : SK.Word16;
      Access_Rights_Field : SK.Word16;
   end record;
   ...
   begin
      ...
      VMX.VMCS_Read (Field => Seg_to_VMCS_Map (Segment_ID).Limit_Field,
                     Value => Value);
      Segment.Limit := SK.Word32'Mod (Value);
      VMX.VMCS_Read (Field => Seg_to_VMCS_Map (Segment_ID).Access_Rights_Field,
                     Value => Value);
      Segment.Access_Rights := SK.Word32'Mod (Value);
   end Save_Segment;
   ...
   begin
      ...
      VMX.VMCS_Write (Field => Seg_to_VMCS_Map (Segment_ID).Limit_Field,
                      Value => SK.Word64 (Segment.Limit));
      VMX.VMCS_Write
        (Field => Seg_to_VMCS_Map (Segment_ID).Access_Rights_Field,
         Value => SK.Word64 (Segment.Access_Rights));
   end Restore_Segment;
   ...
   begin
      ...
      VMX.VMCS_Write (Field => Constants.GUEST_LIMIT_GDTR,
                      Value => SK.Word64 (Descriptors (Id).GDTR.Limit));
      ...
      VMX.VMCS_Write (Field => Constants.GUEST_LIMIT_IDTR,
                      Value => SK.Word64 (Descriptors (Id).IDTR.Limit));
